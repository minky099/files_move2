...
class ModelItem(db.Model):
    ...
    def get_by_name(name):
        try:
            entity = db.session.query(ModelItem).filter_by(name=name).with_for_update().first()
            return entity
        except Exception, e:
            logger.error('Exception:%s', e)
            logger.error(traceback.format_exc())
    ...
    def get_by_dirName(dirName):
        try:
            entity = db.session.query(ModelItem).filter_by(dirName=dirName).with_for_update().first()
            return entity
        except Exception, e:
            logger.error('Exception:%s', e)
            logger.error(traceback.format_exc())
    ...
    def get_by_targetPath(targetPath):
        try:
            entity = db.session.query(ModelItem).filter_by(targetPath=targetPath).with_for_update().first()
            return entity
        except Exception, e:
            logger.error('Exception:%s', e)
            logger.error(traceback.format_exc())
